...
public class UiAutomatorView extends Composite {
    ...
    private static String classNameStatic = null;
    private static String xpathStatic = null;
    ...
    public void loadAttributeTable() {
        ...
        getXpath1(mModel.getSelectedNode().getAttributesArray());
        if(getXpath(mModel.getSelectedNode().getAttributesArray())!=null){
        	copySelectorToClipboard(getXpath(mModel.getSelectedNode().getAttributesArray()));
        }else{
        	copySelectorToClipboard(getClassname(mModel.getSelectedNode().getAttributesArray()));
        }
    }
    
    public String getClassname(Object[] nodeArray){
    	String className = null;
    	if(nodeArray!=null){
	    	for(Object obj:mModel.getSelectedNode().getAttributesArray()){
    	...
    	classNameStatic = className;
    	return className;
    }
    
    public void copySelectorToClipboard(String selectors){
    	StringSelection selection = new StringSelection(selectors);
    	Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
    	clipboard.setContents(selection, selection);
    }
    
    public String getXpath(Object[] nodeArray){
    	String className = null;
    	String xpath = null;
    	if(nodeArray!=null){
	    	className = getClassname(nodeArray);
	    	for(Object obj:mModel.getSelectedNode().getAttributesArray()){
	        	AttributePair pair = (AttributePair) obj;
	        	if(pair.key.equals("resource-id")){
	        		if(!pair.value.equals("")){
		        		String resourceId = pair.value.toString();
		        		resourceId = resourceId.split("/")[1];
		        		xpath = "//"+className+"[contains(@resource-id,'"+resourceId+"')]";
	        		}
	        	}
	        }
    	}
    ...
    public void getXpath1(Object[] nodeArray){
    	if(nodeArray!=null){
	    	String className = null;
	    	String xpath = null;
	    	for(Object obj:mModel.getSelectedNode().getAttributesArray()){
